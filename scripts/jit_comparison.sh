...
EPOCHS="${EPOCHS:-20}"
RUN_DIR="${RUN_DIR:-results/jit_comparison}"
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
IMAGE_NAME="leonardobocchieri/jax-hypernetworks:latest"

run() {
    # $1: extra env (e.g. JAX_DISABLE_JIT=1), $2: output subdir
    local overrides="problem=toy use_wandb=False test_model=False plot_inference=False training.epochs=$EPOCHS hydra.run.dir=$RUN_DIR/$2"
...
speedup=$(awk "BEGIN { printf \"%.1f\", $nojit_time / $jit_time }")
